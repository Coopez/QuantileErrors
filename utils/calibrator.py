"""
Function imported from the original codebase and modified to work with CUDA tensors.
...
from pytorch_lattice.layers.categorical_calibrator import CategoricalCalibrator
...
def initialize_feature_calibrators(
    ...
    """
    calibrators = torch.nn.ModuleDict().cpu()
    calibrators = calibrators.cuda() if torch.cuda.is_available() else calibrators
...
class NumericalCalibrator(ConstrainedModule):
    ...
    def __init__(
        ...
        if self.input_keypoints_type == InputKeypointsType.LEARNED:
            ...
            self._interpolation_logits = torch.nn.Parameter(initial_logits).to(self.device)
